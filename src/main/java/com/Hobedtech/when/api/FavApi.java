...
public class FavApi {
    ...
    @PostMapping
    public ResponseEntity<EventsUsers> save(@RequestBody FavPostDto favPostDto){
        return ResponseEntity.ok(favServiceImpl.save(favPostDto));
    }
    @DeleteMapping
    public ResponseEntity<EventsUsers> delete(@RequestParam Long eventId, Long userId){
        return ResponseEntity.ok(favServiceImpl.delete(eventId,userId));
    }
}
